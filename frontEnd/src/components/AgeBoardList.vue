<template>
  <div class="container">
    ...
    <table class="board-table" v-if="filteredBoards.length">
      ...
      <tbody>
        <tr v-for="ageBoard in filteredBoards" :key="ageBoard.id">
          <td>
...
const filteredBoards = computed(() => {
  ...
  return store.ageBoards.slice(start, end)
})

const totalPages = computed(() => {
  return Math.ceil(store.ageBoards.length / itemsPerPage)
})
...
console.log(isLogin.value) // 로그인 상태 확인
</script>
