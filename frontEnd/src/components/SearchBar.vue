...
export default {
  ...
  data() {
    return {
      ...
      countryInput: {
        ...
        '전라남도': ['강진군', '고흥군', '곡성군', '광양시', '구례군', '나주시', '목포시', '무안군', '보성군', '순천시', '신안군', '여수시', '영광군', '영암군', '완도군', '장성군', '장흥군', '진도군', '함평군', '해남군', '화순군'],
  ...
  methods: {
    searchPlaces() {
      this.$emit("search-places", { province: this.selectedProvince, country: this.selectedCountry, bank: this.selectedBank });
    },
    moveToCurrentLocation() {
      this.$emit("move-to-current-location");
    },
    updateCountryList() {
      this.countryList = this.countryInput[this.selectedProvince];
    }
  },
  watch: {
